...
- [`taze`](https://github.com/Sunny-117/taze): Checks and bumps dependency versions, now supporting reading and updating versions from catalogs.
- [`eslint-plugin-pnpm`](https://github.com/Sunny-117/pnpm-workspace-utils/tree/main/packages/eslint-plugin-pnpm): Enforces using catalogs for all dependencies in `package.json`, with auto-fixes.
  - If you use [`@antfu/eslint-config`](https://github.com/Sunny-117/eslint-config), enable this by setting `pnpm: true`.
- [`pnpm-workspace-yaml`](https://github.com/Sunny-117/pnpm-workspace-utils/tree/main/packages/pnpm-workspace-yaml): A utility library for reading and writing `pnpm-workspace.yaml` while preserving comments and formatting.
- [`node-modules-inspector`](https://github.com/Sunny-117/node-modules-inspector): Visualizes your `node_modules`, now labeling dependencies with their catalog name for a better overview of their origin.
- [`nip`](https://github.com/Sunny-117/nip): Interactive CLI to install packages to catalogs
